...
def show_mi_equipo():
    ...
    with tab1:
        ...
        for agente in mis_agentes:
            ...
            sph_real = round(ventas_periodo / (horas_totales * 0.83), 3) if ventas_periodo > 0 and horas_totales > 0 else 0.0
        ...
        with col4:
            ...
            sph_global = round(total_ventas_equipo / (total_horas_equipo * 0.83), 3) if total_horas_equipo > 0 else 0.0
    ...
    with tab2:
        ...
        if 'editing_venta' not in st.session_state or st.session_state.editing_venta.get('agente') != agente_sel:
            ...
            st.markdown("---")
            st.caption("⭐ Puntos extra asociados a esta venta")
            puntos_extra_venta = st.number_input("Puntos extra", min_value=0, value=0, step=5, key="puntos_extra_venta")
            ...
            if st.button("➕ Añadir Venta", type="primary", use_container_width=True):
                nueva_venta = {
                    ...
                    "puntos_extra_venta": puntos_extra_venta,
                    "campaña": next((a.get('campaign', 'CAPTA') for a in mis_agentes if a['username'] == agente_sel), 'CAPTA'),
                    "supervisor": supervisor
                }
                if puntos_extra_venta > 0:
                    if agente_sel not in datos_puntos['puntos_extra']:
                        datos_puntos['puntos_extra'][agente_sel] = {}
                    if fecha_str not in datos_puntos['puntos_extra'][agente_sel]:
                        datos_puntos['puntos_extra'][agente_sel][fecha_str] = []
                    datos_puntos['puntos_extra'][agente_sel][fecha_str].append({
                        "puntos": puntos_extra_venta,
                        "motivo": f"Extra por venta de {producto}",
                        "campaña": next((a.get('campaign', 'CAPTA') for a in mis_agentes if a['username'] == agente_sel), 'CAPTA'),
                        "supervisor": supervisor
                    })
                    guardar_datos_puntos(datos_puntos)
