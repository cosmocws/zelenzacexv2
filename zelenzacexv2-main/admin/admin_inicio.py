...
def show_inicio_admin():
    ...
    with col_fest1:
        fecha_festivo = st.date_input("📅 Marcar día festivo:", value=hoy, key="fecha_festivo")
    with col_fest2:
        st.write("")
        st.write("")
        if st.button("🎌 Marcar TODOS ausentes", use_container_width=True):
            fecha_f_str = fecha_festivo.strftime('%Y-%m-%d')
            if fecha_f_str not in registro:
                registro[fecha_f_str] = {}
            for agente in um.get_all_agents():
                if not agente.get('standby', False):
                    username = agente['username']
                    registro[fecha_f_str][username] = {
                        'ventas': 0, 'llamadas_5m': 0, 'llamadas_15m': 0,
                        'ausente': True,
                        'campaña': agente.get('campaign', 'CAPTA'),
                        'supervisor': agente.get('manager', '')
                    }
            from super.super_panel import guardar_registro_diario
            guardar_registro_diario(registro)
            st.success(f"✅ {fecha_f_str} marcado como festivo. Todos ausentes.")
            st.rerun()
    ...
    with col_a3:
        ventas_por_agente = round(total_ventas / len(agentes_activos), 1) if agentes_activos else 0
    ...
    for agente in agentes:
        ...
        sph_agente = round(ventas_agente / (horas_efect * 0.83), 3) if ventas_agente > 0 and horas_efect > 0 else 0.0
