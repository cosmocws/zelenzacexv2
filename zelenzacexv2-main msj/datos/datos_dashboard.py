...
def _mostrar_metricas_dia(um, registro, hoy_str):
    ...
    sph_capta = round(ventas_capta / (horas_capta * 0.83), 3) if ventas_capta > 0 and horas_capta > 0 else 0
    sph_winback = round(ventas_winback / (horas_winback * 0.83), 3) if ventas_winback > 0 and horas_winback > 0 else 0
...
def _mostrar_ranking(um, registro, campana, mes_actual, hoy, titulo, color):
    ...
    if ranking:
        ...
        cols = st.columns(5)  # 2 filas de 5
        ...
        for i, r in enumerate(ranking[:10]):
            ...
            with cols[col_idx]:
                ...
                <div class="rank-card {border}">
                    ...
                    <p style="color:white;font-size:14px;">{r['Nombre']}</p>
                    <p style="font-size:36px;font-weight:bold;color:#e94560;">{r['Ventas']} ventas</p>
                    <p style="color:white;font-size:16px;">SPH: {r['SPH']:.3f}</p>
                </div>
                """, unsafe_allow_html=True)
...
def _mostrar_ultimas_ventas(datos_puntos):
    ...
    for username, ventas_agente in datos_puntos['ventas'].items():
        for fecha, ventas_dia in ventas_agente.items():
            for v in ventas_dia:
                pts = v.get('puntos', 0)
                pts_extra = v.get('puntos_extra_venta', 0)
                ultimas.append({
                    'fecha': fecha,
                    'agente': username,
    ...
    if capta_ventas:
        ...
        for i, v in enumerate(capta_ventas):
            with cols[i]:
                pts = v['puntos']
                pts_extra = v['puntos_extra']
                euros = (pts + pts_extra) / 22
                st.markdown(f"""
                <div class="venta-card capta-venta">
                    <h3>{v['agente']}</h3>
                    <p style="color:white;font-size:18px;">{v['producto']}</p>
                    <p style="color:#e94560;font-size:22px;">{pts} pts</p>
                    <p style="color:#ffc107;font-size:14px;">⭐ +{pts_extra} extra</p>
                    <p style="color:white;font-size:14px;">💰 {euros:.2f}€</p>
                </div>
                """, unsafe_allow_html=True)
...
def _calcular_ranking(um, registro, campana, mes_actual, hoy):
    ...
    año_mes, mes_mes = int(mes_actual[:4]), int(mes_actual[5:7])
    dias_en_mes = monthrange(año_mes, mes_mes)[1]
    dia_fin = hoy.day if mes_actual == hoy.strftime('%Y-%m') else dias_en_mes
    ...
    for a in todos_agentes:
        ...
        dia_inicio = 1
        if mes_actual == a.get('incorporation_date', '')[:7]:
